...
const GlobalSearch = () => {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      customFieldMatches?.forEach((match: any) => {
        if (!addedLeadIds.has(match.lead_id)) {
          ...
          searchResults.push({
            ...
            leadStatusId: lead.status_id,
            type: "lead",
            matchedContent: `${match.field_key}: ${match.value}`,
            status: lead.status,
            icon: <User className="h-4 w-4" />,
          });
      ...
      if (sessions && sessions.length > 0) {
        ...
        const sessionLeadMap = new Map(
          sessionLeads?.map((l) => [l.id, l]) || []
        );
      ...
      if (projects && projects.length > 0) {
        const projectLeadIds = [
          ...new Set(projects.map((p: any) => p.lead_id)),
        ];
