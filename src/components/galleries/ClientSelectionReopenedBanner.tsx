...
type ClientSelectionReopenedBannerProps = {
  note?: string | null;
  className?: string;
};

export function ClientSelectionReopenedBanner({ note, className }: ClientSelectionReopenedBannerProps) {
    ...
    <section className={cn("rounded-2xl border border-emerald-100 bg-emerald-50 p-4 md:p-6", className)}>
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:gap-4">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700">
          <CircleDashed className="h-6 w-6" strokeWidth={2.5} />
        </div>

        <div className="space-y-2">
          <div className="space-y-1">
            <h3 className="text-lg font-bold text-emerald-900 md:text-xl">
              {t("sessionDetail.gallery.clientPreview.reopenedBanner.title")}
            </h3>
            <p className="text-sm font-semibold text-emerald-900/80 md:text-base">
              {t("sessionDetail.gallery.clientPreview.reopenedBanner.subtitle")}
            </p>
          </div>

          <p className="whitespace-pre-line text-sm leading-relaxed text-emerald-900/70 md:text-base">
            {t("sessionDetail.gallery.clientPreview.reopenedBanner.body")}
          </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-extrabold uppercase tracking-widest text-emerald-800">
                    {t("sessionDetail.gallery.clientPreview.reopenedBanner.noteLabel")}
                  </p>
