...
describe("GalleryDetailSettings", () => {
  ...
  it("hides watermark config sections when watermark is disabled", () => {
    render(
      <SettingsProvider>
        <GallerySettingsContent
          activeTab="watermark"
    ...
    expect(screen.getByRole("switch", { name: "Watermark" })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Appearance" })).not.toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Placement & style" })).not.toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Live preview" })).not.toBeInTheDocument();
  });

  it("shows watermark config sections when enabled and updates option cards", () => {
      ...
      <SettingsProvider>
        <GallerySettingsContent
          ...
          watermark={{
            ...
            businessName: "Studio",
            logoUrl: null,
            previewBackgroundUrl: "https://example.com/preview.jpg",
            onOpenOrganizationBranding: jest.fn(),
          }}
          privacy={{
            settings: { passwordEnabled: false },
    ...
    expect(screen.getByRole("heading", { name: "Placement & style" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Live preview" })).toBeInTheDocument();

    const layoutHeading = screen.getByRole("heading", { name: "Placement & style" });
    const previewHeading = screen.getByRole("heading", { name: "Live preview" });
    expect(layoutHeading.compareDocumentPosition(previewHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();

    fireEvent.click(screen.getByRole("button", { name: "Logo" }));
    expect(onSettingsChange).toHaveBeenCalledWith({ type: "logo" });

    fireEvent.click(screen.getByRole("button", { name: "Corner" }));
    expect(onSettingsChange).toHaveBeenCalledWith({ placement: "corner" });
