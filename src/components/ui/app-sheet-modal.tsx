...
export function AppSheetModal({
  ...
  const sheetContentClass = cn("flex flex-col",
  ...
  // Size variants
  size === 'content' && !isMobile && "sm:max-w-md", size === 'lg' && !isMobile && "sm:max-w-2xl");
  return <Sheet open={isOpen} onOpenChange={handleOpenChange}>
      <SheetContent side={sideVariant} className={cn(sheetContentClass, "[&>button]:hidden")} onPointerDownOutside={e => {
      ...
      const target = e.target as HTMLElement;
      if (target && (target.closest('[data-radix-popper-content-wrapper]') || target.closest('[data-radix-select-content]') || target.closest('[data-radix-popover-content]'))) {
        e.preventDefault();
        return;
      }
      if (dirty && onDirtyClose) {
        ...
        {footerActions.length > 0 && <SheetFooter className="border-t pt-4 gap-2 px-1">
            {footerActions.map((action, index) => <Button key={index} variant={action.variant || (index === footerActions.length - 1 ? 'default' : 'outline')} onClick={action.onClick} disabled={action.disabled || action.loading} className={cn("flex-1", action.loading && "cursor-not-allowed")}>
                {action.loading ? <div className="flex items-center gap-2">
