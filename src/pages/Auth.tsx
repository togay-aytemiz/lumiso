...
import LegalDocumentsModal from "@/components/auth/LegalDocumentsModal";
import {
  getLegalVersions,
...
const getModeFromLocation = (
  ...
  if (
    normalized.endsWith("/auth/signup") ||
    normalized.endsWith("/auth/sign-up")
  ) {
    return "signup";
  }
...
const getAuthIntentFromLocation = (
  pathname: string,
  search: string,
  hash: string
) => {
...
const Auth = () => {
  ...
  const [legalVersions, setLegalVersions] = useState<LegalVersionsMap | null>(
    null
  );
  const [legalVersionsLoading, setLegalVersionsLoading] = useState(false);
  const [legalVersionsError, setLegalVersionsError] = useState<string | null>(
    null
  );
  ...
  const authIntent = getAuthIntentFromLocation(
    location.pathname,
    location.search,
    location.hash || ""
  );
  const recoveryIntentActive = authIntent === "recovery";
  const openLegalLink = (doc: LegalDocumentId) => {
    const href =
      doc === "terms"
  ...
  useEffect(() => {
    const derivedMode =
      recoveryIntentActive || isPasswordResetRequestMode
        ? "signin"
  ...
  useEffect(() => {
    ...
    getLegalVersions()
      .then((data) => {
        if (!isMounted) return;
        setLegalVersions(data);
      })
      .catch((error) => {
        console.error("Failed to fetch legal versions", error);
        if (!isMounted) return;
  ...
  }, [isSignUp, legalVersions, legalVersionsLoading, legalVersionsError]);

  useEffect(() => {
    if (acceptsTerms && termsError) {
      setTermsError(null);
    }
  }, [acceptsTerms, termsError]);
  ...
  const featureSlides = [
    ...
    },
    {
      ...
      title: tPages(
        "auth.features.2.title",
        "Keep your shoot schedule crystal clear"
      ),
      description: tPages(
  ...
  const currentFeatureChip =
    featureSlides[activeFeatureIndex]?.chip ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isSignUp && !acceptsTerms) {
      setTermsError(
        "Hizmet Şartları ve Gizlilik Politikasını kabul etmeniz gerekmektedir."
      );
      return;
    }
    ...
    setTermsError(null);
    setLoading(true);
    logAuthEvent(isSignUp ? "auth_sign_up_start" : "auth_sign_in_start", {
      email,
    });
      ...
      if (isSignUp) {
        ...
        const buildLegalConsentsPayload = (): LegalConsentsPayload => {
          ...
          return {
            ...
            privacy: { version: versionFor("privacy"), acceptedAt: timestamp },
            kvkk: { version: versionFor("kvkk"), acceptedAt: timestamp },
  ...
  const handlePasswordResetRequest = async (event?: React.FormEvent) => {
    ...
    try {
      ...
      logAuthEvent("auth_reset_request_start", {
        email,
      ...
      recordToast(
        "error",
        error instanceof Error && error.message ? undefined : fallbackKey,
        message
      );
    } finally {
  ...
  const handleResendVerification = async () => {
    ...
    if (!targetEmail) {
      const copy = tMsg(
        ...
        "Doğrulama bağlantısını tekrar göndermek için e-postanızı girin."
      );
      toast.error(copy);
  ...
  const verificationEmailDisplay =
    verificationTargetEmail || tForm("placeholders.enter_email");
      ...
      <div className="flex min-h-screen w-full bg-white">
        <div className="relative flex min-h-screen w-full flex-col overflow-hidden lg:flex-row">
          <div className="relative flex min-h-screen w-full flex-col px-6 py-8 pb-12 sm:px-10 sm:pb-12 lg:w-1/2 lg:px-12 lg:pb-10">
            <div
              className="absolute inset-x-0 top-0 h-24 bg-gradient-to-br from-primary/10 via-white to-transparent"
              aria-hidden="true"
            />
            ...
            <div className="absolute left-6 top-6 sm:left-10">
              <img
                src="/lumiso-logo.png"
                alt={tCommon("branding.app_name", "Lumiso")}
            ...
            <div className="flex flex-1 flex-col">
              ...
              <div className="flex flex-1 items-start pt-16 sm:pt-20 lg:items-center lg:pt-0">
                <div
                  key={viewKey}
                  className="relative mx-auto w-full max-w-md fade-in-up"
                >
                  <div className="mb-8 space-y-3">
                    ...
                    {isVerificationPending ? (
                      <p className="text-base text-slate-600">
                        ...
                        {tPages(
                          "auth.verification.copyAfter",
                          "adresine gönderdik; çalışma alanını etkinleştirmek için onayla."
                        )}
                      </p>
                    ) : (
                  ...
                  {isPasswordResetMode ? (
                    <form onSubmit={handlePasswordUpdate} className="space-y-6">
                      <p className="text-sm text-slate-500">
                        {tForm("auth.password_reset.instructions")}
                      </p>
                      <div className="space-y-2">
                        ...
                        <div className="mt-2">
                          ...
                          <div className="mt-1 flex justify-between text-[10px] text-slate-500">
                            ...
                            <span>
                              {resetPwdStrength.label === "weak" &&
                                tForm("password_tips.weak", "Weak")}
                              {resetPwdStrength.label === "fair" &&
                                tForm("password_tips.fair", "Fair")}
                              {resetPwdStrength.label === "good" &&
                                tForm("password_tips.good", "Good")}
                              {resetPwdStrength.label === "strong" &&
                                tForm("password_tips.strong", "Strong")}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      >
                        {updatingPassword
                          ? `${tMsg("info.loading")}...`
                          : tForm("auth.password_reset.button")}
                      </Button>
                    </form>
                  ) : isPasswordResetRequestMode ? (
                    <form
                    ...
                    <>
                      <div className="flex flex-wrap gap-3">
                        ...
                        >
                          {tPages(
                            "auth.verification.actions.useAnother",
                            "Use another email"
                          )}
                        </Button>
                      </div>
                              ...
                              <p className="text-sm text-slate-700">
                                ...
                                <button
                                  ...
                                  onClick={(event) => {
                                    ...
                                    openLegalLink("communication-consent");
                                  }}
                                >
                                  ilgili bilgilendirme metnini
                                </button>{" "}
                                kabul ediyorum.
                              </p>
                            </label>
                      ...
                      <div className="mt-8 flex flex-wrap items-center gap-x-2 text-sm text-slate-500">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setShowPassword(false);
                            navigate(
                              nextIsSignUp ? "/auth/signup" : "/auth/signin"
                            );
                          }}
                          disabled={loading}
                          className="px-0 text-sm font-semibold text-primary hover:text-primary/80"
              ...
              <div className="hidden pt-6 text-xs text-slate-400 sm:block">
                <span>
                  ...
                  {tCommon("branding.app_name", "Lumiso")}.{" "}
                  {tCommon("legal.all_rights_reserved", "All rights reserved.")}
                </span>
              </div>
            </div>
          </div>
