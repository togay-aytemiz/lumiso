...
-- Keep the first default and unset others
...
-- Add a unique constraint to prevent multiple defaults per user
-- This creates a partial unique index that only applies to rows where is_default = true
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_project_types_single_default 
ON project_types (user_id) 
WHERE is_default = true;