-- Fix sessions RLS policies to use organization_id instead of user_id lookup
-- This will allow team members to see all sessions within their organization

-- First, add organization_id column to sessions table
...
-- Update existing sessions without lead_id to use their user's active organization
UPDATE public.sessions 
SET organization_id = (
...
-- Update settings tables to be organization-based instead of user-based
-- Add organization_id to all settings tables
