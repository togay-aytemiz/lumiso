...
-- Fixed to handle user context properly
...
BEGIN
    ...
    -- Create 10 projects linked to leads
    WITH new_projects AS (
        ...
        VALUES 
            ...
            (current_user_id, org_id, lead_ids[9], 'Wilson Spring Wedding', 'Elegant spring wedding at Riverside Manor. Premium package with videography.', wedding_type_id, progress_proj_status_id, ARRAY[current_user_id], '2024-05-15', now() - interval '10 days', now() - interval '1 day'),
            (current_user_id, org_id, lead_ids[11], 'Brown Winter Wedding', 'Completed winter wedding with mountain backdrop. Premium album delivered.', wedding_type_id, completed_proj_status_id, ARRAY[current_user_id], '2023-12-15', now() - interval '45 days', now() - interval '5 days'),
            ...
            (current_user_id, org_id, lead_ids[10], 'Baby Elif Newborn Session', 'Studio newborn session with custom setup. Family shots included.', newborn_type_id, progress_proj_status_id, ARRAY[current_user_id], '2024-02-25', now() - interval '12 days', now() - interval '3 days'),
            ...
            (current_user_id, org_id, lead_ids[3], 'TechCorp Headshots', 'Professional headshots for 15 employees. Studio and environmental options.', corporate_type_id, planned_proj_status_id, ARRAY[current_user_id], '2024-03-01', now() - interval '3 hours', now() - interval '3 hours'),
            (current_user_id, org_id, lead_ids[12], 'Innovation Labs Executive Portraits', 'Completed executive team portraits with branding elements.', corporate_type_id, completed_proj_status_id, ARRAY[current_user_id], '2023-12-20', now() - interval '30 days', now() - interval '7 days'),
            (current_user_id, org_id, lead_ids[7], 'Amanda Professional Headshots', 'LinkedIn-ready professional portraits with multiple outfit changes.', portrait_type_id, planned_proj_status_id, ARRAY[current_user_id], '2024-02-28', now() - interval '6 days', now() - interval '2 days')
        RETURNING id
    )
    SELECT ARRAY_AGG(id) INTO project_ids FROM new_projects;

    -- Create 12 sessions linked to projects
    ...
    VALUES 
        ...
        (current_user_id, org_id, lead_ids[10], project_ids[6], '2024-02-20', '15:00:00', confirmed_session_status_id, 'Studio A - Newborn Setup', 'Baby Elif newborn session. Parents want natural lighting setup.', now() - interval '12 days', now() - interval '3 days'),
        (current_user_id, org_id, lead_ids[3], project_ids[8], '2024-03-01', '09:00:00', confirmed_session_status_id, 'TechCorp Office - Conference Room B', '15 employee headshots. Professional attire. 2-hour session.', now() - interval '3 hours', now() - interval '3 hours'),
        ...
        (current_user_id, org_id, lead_ids[5], project_ids[5], '2024-02-10', '17:30:00', editing_session_status_id, 'Sunset Point - Overlook Trail', 'Beautiful maternity session completed. 150+ photos taken.', now() - interval '4 days', now() - interval '2 days'),
        (current_user_id, org_id, lead_ids[7], project_ids[10], '2024-02-08', '11:00:00', editing_session_status_id, 'Studio B - Professional Setup', 'Professional headshots completed. 3 outfit changes, multiple backgrounds.', now() - interval '6 days', now() - interval '2 days'),
        
        -- Completed sessions
        (current_user_id, org_id, lead_ids[11], project_ids[4], '2023-12-15', '13:00:00', completed_session_status_id, 'Mountain Lodge - Great Hall', 'Beautiful winter wedding. Snowy backdrop perfect for ceremony shots.', now() - interval '45 days', now() - interval '5 days'),
        (current_user_id, org_id, lead_ids[12], project_ids[9], '2023-12-20', '10:00:00', completed_session_status_id, 'Innovation Labs - Executive Boardroom', 'Executive team portraits completed. Professional lighting, corporate branding.', now() - interval '30 days', now() - interval '7 days'),
        ...
        (current_user_id, org_id, lead_ids[9], project_ids[3], '2024-01-20', '15:30:00', completed_session_status_id, 'Riverside Manor - Gardens', 'Engagement photos at wedding venue. Couple loved the location scouting.', now() - interval '10 days', now() - interval '1 day'),
        
        -- Cancelled session  
        (current_user_id, org_id, lead_ids[13], NULL, '2024-01-15', '14:00:00', cancelled_session_status_id, 'Originally: Beach Sunset Location', 'Session cancelled due to client schedule changes.', now() - interval '25 days', now() - interval '15 days');
