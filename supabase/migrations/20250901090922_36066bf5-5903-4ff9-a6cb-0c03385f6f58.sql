-- Final corrected photography business seed data
-- Using only 'planned' status for sessions to avoid enum errors
...
BEGIN
    ...
    -- Create 8 sessions linked to projects (using only 'planned' status to avoid enum errors)
    ...
    VALUES 
        ...
        (current_user_id, org_id, lead_ids[10], project_ids[6], '2024-02-20', '15:00:00', 'planned', 'Studio A - Newborn Setup', 'Baby Elif newborn session. Parents want natural lighting setup.', now() - interval '12 days', now() - interval '3 days'),
        (current_user_id, org_id, lead_ids[3], project_ids[8], '2024-03-01', '09:00:00', 'planned', 'TechCorp Office - Conference Room B', '15 employee headshots. Professional attire. 2-hour session.', now() - interval '3 hours', now() - interval '3 hours'),
        ...
        -- Past sessions
        (current_user_id, org_id, lead_ids[11], project_ids[4], '2023-12-15', '13:00:00', 'planned', 'Mountain Lodge - Great Hall', 'Beautiful winter wedding. Snowy backdrop perfect for ceremony shots.', now() - interval '45 days', now() - interval '5 days'),
        (current_user_id, org_id, lead_ids[12], project_ids[9], '2023-12-20', '10:00:00', 'planned', 'Innovation Labs - Executive Boardroom', 'Executive team portraits completed. Professional lighting, corporate branding.', now() - interval '30 days', now() - interval '7 days'),
        (current_user_id, org_id, lead_ids[8], project_ids[2], '2024-02-14', '17:00:00', 'planned', 'Downtown Arts District', 'Engagement session for Davis couple. Urban backdrop with vintage feel.', now() - interval '7 days', now() - interval '1 day');
    ...
    RAISE NOTICE 'Successfully created comprehensive seed data: % leads, % projects, % sessions, % activities, % todos', 
        array_length(lead_ids, 1), array_length(project_ids, 1), 8, 7, 12;
