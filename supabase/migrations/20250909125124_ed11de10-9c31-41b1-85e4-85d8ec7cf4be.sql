...
-- Create improved session workflow trigger function with case-insensitive matching
...
BEGIN
  ...
  IF OLD.status IS NOT NULL THEN
    ...
    -- Log for debugging
    RAISE LOG 'Old status: %, Old lifecycle: %', OLD.status, COALESCE(old_lifecycle, 'NOT FOUND');
  END IF;
  ...
  -- Log for debugging
  RAISE LOG 'New status: %, New lifecycle: %', NEW.status, COALESCE(new_lifecycle, 'NOT FOUND');
  ...
  IF old_lifecycle IS DISTINCT FROM new_lifecycle AND new_lifecycle IS NOT NULL THEN
    ...
    RAISE LOG 'Lifecycle changed from % to %, trigger type: %', 
              COALESCE(old_lifecycle, 'NULL'), 
              new_lifecycle, 
              COALESCE(workflow_trigger_type, 'NONE');
    ...
    IF workflow_trigger_type IS NOT NULL THEN
      ...
      RAISE LOG 'Workflow executor called for session % with trigger %', NEW.id, workflow_trigger_type;
    END IF;
  END IF;
