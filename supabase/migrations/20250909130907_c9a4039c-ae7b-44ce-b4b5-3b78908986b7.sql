-- Fix parameter mapping in session workflow trigger function
...
-- Create session workflow trigger function with correct parameter names
...
BEGIN
  ...
  IF old_lifecycle IS DISTINCT FROM new_lifecycle AND new_lifecycle IS NOT NULL THEN
    ...
    IF workflow_trigger_type IS NOT NULL THEN
      -- Call the workflow executor edge function with correct parameter names
