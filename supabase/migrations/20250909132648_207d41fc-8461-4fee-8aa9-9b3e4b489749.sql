-- Phase 1: Critical Workflow System Fixes
...
-- Create workflow execution fingerprinting for better duplicate prevention
...
AS $$
BEGIN
  -- Create a fingerprint based on workflow, entity, and relevant trigger data
  RETURN encode(
    digest(
...
CREATE INDEX IF NOT EXISTS idx_workflow_executions_fingerprint 
ON workflow_executions(
  get_workflow_execution_fingerprint(
    workflow_id, 
    trigger_entity_type, 
    trigger_entity_id,
    COALESCE((execution_log->-1->>'trigger_data')::JSONB, '{}'::JSONB)
  )
