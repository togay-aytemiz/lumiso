-- Fix the schedule_session_reminders function to remove non-existent updated_at column reference
...
BEGIN
  ...
  SELECT s.*, o.timezone as org_timezone
  INTO session_record
  FROM sessions s
  ...
  SELECT COALESCE(os.timezone, 'UTC') as timezone
  INTO org_record
  ...
  SELECT (session_record.session_date + session_record.session_time) AT TIME ZONE COALESCE(org_record.timezone, 'UTC') AT TIME ZONE 'UTC' as session_datetime_utc
  INTO session_record;

  RAISE LOG 'Session datetime in UTC: %, org timezone: %', session_record.session_datetime_utc, COALESCE(org_record.timezone, 'UTC');
  ...
  LOOP
    ...
    BEGIN
      ...
      reminder_datetime := session_record.session_datetime_utc - (delay_minutes || ' minutes')::INTERVAL;
      
      -- If reminder time is in the past but very recent (less than 5 minutes ago), schedule immediately
      IF reminder_datetime < current_utc AND (current_utc - reminder_datetime) < INTERVAL '5 minutes' THEN
        reminder_datetime := current_utc + INTERVAL '30 seconds'; -- Schedule for immediate processing
        RAISE LOG 'Scheduling immediate reminder for workflow % (original time: %, scheduled for: %)', workflow_name, session_record.session_datetime_utc - (delay_minutes || ' minutes')::INTERVAL, reminder_datetime;
      END IF;
      
      -- Only schedule if reminder time is in the future or immediate
      IF reminder_datetime >= current_utc THEN
        -- Insert or update the scheduled reminder (removed updated_at reference)
        ...
        ON CONFLICT (session_id, workflow_id) 
        DO UPDATE SET
