-- Extend storage usage to count originals when they are stored.
-- Proof galleries only count proofSize; final/delivery galleries also count originalSize when storage_path_original is set.
  ...
  gallery_sum as (
    select coalesce(
      sum(
        ...
        case
          when ga.status = 'ready'
            and ga.storage_path_original is not null
            and ga.metadata ? 'originalSize'
            and (ga.metadata->>'originalSize') ~ '^[0-9]+$'
            then (ga.metadata->>'originalSize')::bigint
          else 0
        end
      ),
      0
