-- Ensure signup never touches the removed team tables
-- Simplify membership checks and new-user bootstrap flow for single-member organizations

-- user_has_pending_membership used to SELECT from organization_members, which no longer exists.
-- Keep the function but make it a safe stub so any RPC callers get a deterministic false result.
...
-- Rebuild the handle_new_user_organization trigger function so it only relies on organizations
-- and other still-existing tables. This prevents signup from trying to touch organization_members
-- or the removed active_organization_id column.
