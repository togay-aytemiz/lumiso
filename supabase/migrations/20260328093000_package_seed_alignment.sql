...
-- Align existing project types with localized defaults for TR orgs still showing English labels
WITH tr_orgs AS (
...
FROM tr_orgs
JOIN public.default_project_type_templates AS tr_defaults
  ON tr_defaults.slug = pt.template_slug
 AND tr_defaults.locale = 'tr'
JOIN english_defaults AS en_defaults
  ON en_defaults.slug = pt.template_slug
WHERE pt.organization_id = tr_orgs.organization_id
  AND pt.template_slug IS NOT NULL
  AND pt.name = en_defaults.en_name;

-- Align existing session types with localized defaults for TR orgs
WITH tr_orgs AS (
  SELECT organization_id
  FROM public.organization_settings
  WHERE public.normalize_locale_code(preferred_locale, 'tr') = 'tr'
...
FROM tr_orgs
JOIN public.default_session_type_templates AS tr_defaults
  ON tr_defaults.slug = st.template_slug
 AND tr_defaults.locale = 'tr'
JOIN english_session_defaults AS en_defaults
  ON en_defaults.slug = st.template_slug
WHERE st.organization_id = tr_orgs.organization_id
