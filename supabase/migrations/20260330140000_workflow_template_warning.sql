-- Seed workflows without automatically attaching templates so users can choose intentionally
...
-- Remove template bindings from seeded workflows so users see the missing-template warning
UPDATE public.workflow_steps ws
SET action_config = action_config - 'template_id'
WHERE ws.action_type = 'send_notification'
  AND ws.action_config ? 'template_id'
  AND (ws.action_config->>'template_slug') IN ('session_confirmation','session_reminder');
