-- Re-runable fix: ensure Seray demo auth user and membership exist.
-- Use a fresh migration name so Supabase applies it even if previous attempt was already marked.
...
BEGIN
  ...
  -- Ensure user_settings points to org if column exists.
  IF EXISTS (
    SELECT 1
