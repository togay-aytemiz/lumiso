-- Ensure sample data projects cover proposal stage alongside planned/in-progress and prefix sample leads.
...
BEGIN
  ...
  LOOP
    ...
    END;
    IF NOT (
      lead_name ILIKE tr_notes_prefix || '%'
      OR lead_name ILIKE en_notes_prefix || '%'
    ) THEN
...
-- Backfill existing sample leads so names carry the sample prefix alongside projects/services.
DO $$
DECLARE
  ...
  SET name = CASE
    WHEN COALESCE(
      (SELECT public.normalize_locale_code(os.preferred_locale, 'tr')
       ...
       LIMIT 1),
      'tr'
    ) LIKE 'tr%' THEN tr_sample_prefix || l.name
    ELSE en_sample_prefix || l.name
  END
  WHERE (
    l.notes ILIKE '%' || tr_sample_prefix || '%'
    OR l.notes ILIKE '%' || en_sample_prefix || '%'
  )
  AND NOT (
    l.name ILIKE tr_sample_prefix || '%'
    OR l.name ILIKE en_sample_prefix || '%'
  );
END $$;
